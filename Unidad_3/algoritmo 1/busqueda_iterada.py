...
class Energia:
    ...
    def perturbar(self, solucion):
        vector = solucion.copy()
        clave = list(vector.keys())[0]
        vector[clave] = vector[clave]  # Mantiene el primer valor sin cambios
        for i in list(vector.keys())[1:]:
            vector[i] = rnd.randint(self.preferencias_energia[i][0], self.preferencias_energia[i][1])
        return vector
...
if __name__ == "__main__":
    ...
    while it_ils < max_it_ils:
        it_local = 0
        while it_local < max_it_local:
            ...
            if vo_temporal < best_vo:
                ...
                print("Nueva mejor sol:", best_sol, "Ganancia:", best_vo)
            it_local += 1

        best_sol = satis.perturbar(best_sol)
        vo_temporal = satis.calcula_ganancia_satisfaccion(satis.calcula_energia(best_sol, valoresActuales))
        if vo_temporal < best_vo:
            best_vo = vo_temporal
            print("Nueva solucion :", best_sol, "Ganancia:", best_vo)

        it_ils += 1

    print("Mejor solucion:", best_sol)
